...
const pressureLesson: Record<HackTrigger, { question: string; reflex: string }> = {
  ...
  K: { question: "Did you stop before the irreversible click, transfer, code, credential or reply?", reflex: "Use the kill-switch: stop the requested action, leave the channel, then verify from a clean starting point." },
};
...
export default async function FeedbackPage({ params }: { params: { sessionId: string } }) {
  ...
  const card = await prisma.challengeSessionCard.findUnique({
    ...
    select: {
      ...
      scenario: {
        select: {
          hackKey: true, explanation: true, proTip: true,
  ...
  const feedback = {
    4: { label: "STRONG MOVE", title: "You broke the pressure chain.", body: "You took control of the next step away from the requester and put verification back in your hands.", tone: "strong" },
    ...
    0: { label: "PRESSURE WON", title: "That is the move the scam needs.", body: "The request still controls the link, payment, credential or next action. That is where the risk compounds.", tone: "risk" },
  }[score as 0 | 1 | 2 | 3 | 4];
  ...
  return (
    <main className="page">
      <section className="shell">
        <header className="topbar"><Link href="/" className="brand">Konfydence</Link><span>{session.currentIndex} / {totalCards}</span></header>

        <section className={`feedback ${feedback.tone}`}>
          <div className="resultHead">
            <div><p className="label">{feedback.label}</p><h1>{feedback.title}</h1><p className="body">{feedback.body}</p></div>
            <div className="score">{score}<span>/4</span></div>
          </div>

          <div className="pressureBlock">
            <div className="pressureIcon"><HackIcon trigger={hackKey} size={20} color="#091522" /></div>
