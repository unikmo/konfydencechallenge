...
import { LOGIN_EMAIL_STRINGS } from "@/lib/challenge/accountStrings";
...
export function renderLoginCodeEmail(input: { code: string; magicLinkUrl: string; lang?: UiLang }): { subject: string; html: string } {
  const lang = input.lang ?? "en";
  const t = LOGIN_EMAIL_STRINGS[lang];
  const subject = t.subject(input.code);
  const spaced = input.code.split("").join(" ");
  const html = `<!DOCTYPE html>
<html lang="${lang}">
<head><meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1" /><title>${escapeHtml(subject)}</title></head>
<body style="margin:0;padding:0;background:#F6F3EC;">
  <span style="display:none!important;visibility:hidden;opacity:0;height:0;width:0;overflow:hidden;">${escapeHtml(t.preheader(input.code))}</span>
    ...
    <tr><td align="center" style="padding:32px 16px;">
      <table role="presentation" width="480" cellpadding="0" cellspacing="0" border="0" style="width:480px;max-width:100%;">
        ...
        <tr><td style="background:#FFFFFF;border:1px solid #E6E1D6;border-radius:16px;padding:30px;">
          <p style="margin:0 0 6px;font:400 14px/1.6 ${BODY_FONT};color:#3B3A36;">${escapeHtml(t.enterCode)}</p>
          <p style="margin:0 0 4px;font:700 30px/1.2 ${BODY_FONT};letter-spacing:.28em;color:#14171A;">${escapeHtml(spaced)}</p>
          <p style="margin:0 0 22px;font:400 12px/1.6 ${BODY_FONT};color:#6E6B64;">${escapeHtml(t.expiresNote)}</p>
          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
            <tr><td align="center" style="border-radius:10px;background:#14171A;">
              <a href="${escapeHtml(input.magicLinkUrl)}" target="_blank" style="display:inline-block;padding:12px 24px;font:700 14px/1 ${BODY_FONT};color:#FFFDF9;text-decoration:none;">${escapeHtml(t.tapToSignIn)}</a>
            </td></tr>
          </table>
          <p style="margin:20px 0 0;font:400 12px/1.6 ${BODY_FONT};color:#94908A;">${escapeHtml(t.ignoreNote)}</p>
        </td></tr>
        <tr><td style="padding:18px 6px 0;font:400 11px/1.7 ${BODY_FONT};color:#94908A;">
          <a href="${escapeHtml(APP_URL)}" target="_blank" style="color:#94908A;text-decoration:none;">konfydence.com</a> · ${escapeHtml(t.tagline)}
        </td></tr>
      </table>
    </td></tr>
  </table>
